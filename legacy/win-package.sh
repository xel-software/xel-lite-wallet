...
PACKAGE=build-client-${VERSION}.zip
...
echo zip
zip -q -X -r ${PACKAGE} build -x \*/.idea/\* \*/.gitignore \*/.git/\* \*/\*.log \*.iml build/conf/build.properties build/conf/logging.properties build/conf/localstorage/\*
rm -rf build
echo done
