...
then
echo obfuscate
/opt/proguard/bin/proguard.sh @build.pro
...
cd -
zip -q -X -r ${PACKAGE}.zip build -x \*/.idea/\* \*/.gitignore \*/.git/\* \*/\*.log \*.iml build/conf/build.properties build/conf/logging.properties build/conf/localstorage/\*
rm -rf build
